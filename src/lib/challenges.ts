import type { Skill } from "@/lib/db/orbita-db";
import { dateKey, weekKey } from "@/lib/streak";
import { selectMixedQuestions } from "@/lib/mastery";
import type { Country } from "@/types/country";

/** Mulberry32 — deterministic PRNG, seedable from a string. */
export function mulberry32(seed: number): () => number {
  let a = seed >>> 0;
  return function () {
    a = (a + 0x6d2b79f5) | 0;
    let t = a;
    t = Math.imul(t ^ (t >>> 15), t | 1);
    t ^= t + Math.imul(t ^ (t >>> 7), t | 61);
    return ((t ^ (t >>> 14)) >>> 0) / 4294967296;
  };
}

export function hashString(s: string): number {
  let h = 2166136261;
  for (let i = 0; i < s.length; i++) {
    h ^= s.charCodeAt(i);
    h = Math.imul(h, 16777619);
  }
  return h >>> 0;
}

export interface ChallengeSet {
  kind: "daily" | "weekly";
  periodKey: string;
  count: number;
  items: Array<{ country: Country; skill: Skill }>;
}

export async function generateDaily(now = Date.now()): Promise<ChallengeSet> {
  const key = dateKey(now);
  const rng = mulberry32(hashString(`daily:${key}`));
  const items = await selectMixedQuestions(
    10,
    ["name", "flag", "capital", "location"],
    { rng },
  );
  return { kind: "daily", periodKey: key, count: items.length, items };
}

export async function generateWeekly(now = Date.now()): Promise<ChallengeSet> {
  const key = weekKey(now);
  const rng = mulberry32(hashString(`weekly:${key}`));
  const items = await selectMixedQuestions(
    25,
    ["name", "flag", "capital", "location"],
    { rng },
  );
  return { kind: "weekly", periodKey: key, count: items.length, items };
}
